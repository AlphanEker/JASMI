...
public class PharmacyMainPage extends JFrame {
	...
	public PharmacyMainPage( Pharmacy pharmacy) {
		...
		comboBox.setModel(new DefaultComboBoxModel(new String[] {pharmacy.employees.get(0).getName(), pharmacy.employees.get(1).getName(),
		pharmacy.employees.get(2).getName()}));
		...
		ActionListener cbActionListener = new ActionListener() {//add action listener to listen for change
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                String s = (String) comboBox.getSelectedItem();//get the selected item
	                ...
	                if (s.equals(pharmacy.employees.get(0).getName()))
	                {
	                 
	        				EmployeeDetailsPage frame1 = new EmployeeDetailsPage(pharmacy.employees.get(0), pharmacy);
	        				frame1.setVisible(true);
	        				Window win1 = SwingUtilities.getWindowAncestor(contentPane);
	        				win1.dispose();
	                }
	                        
	                if (s.equals(pharmacy.employees.get(1).getName()))
	                {
	                 
	        				EmployeeDetailsPage frame1 = new EmployeeDetailsPage(pharmacy.employees.get(1), pharmacy);
	        				frame1.setVisible(true);
	        				Window win1 = SwingUtilities.getWindowAncestor(contentPane);
	        				win1.dispose();
	                }
	                
	                if (s.equals(pharmacy.employees.get(2).getName()))
	                {
	                 
	        				EmployeeDetailsPage frame1 = new EmployeeDetailsPage(pharmacy.employees.get(2), pharmacy);
	        				frame1.setVisible(true);
	        				Window win1 = SwingUtilities.getWindowAncestor(contentPane);
	        				win1.dispose();
	                }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (s) {//check for a match
                    ...
                    case "Available Resources":
        				PharmacyStocksPage frame2 = new PharmacyStocksPage(pharmacy);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (s) {//check for a match
                    case "Change Password":
        				ChangePasswordPage frame1 = new ChangePasswordPage(pharmacy);
        				frame1.setVisible(true);
        				Window win1 = SwingUtilities.getWindowAncestor(contentPane);
        				win1.dispose();
                        break;
                    ...
                    case "Change E-mail":
        				ChangeMailPage frame2 = new ChangeMailPage(pharmacy);
        				frame2.setVisible(true);
        				Window win2 = SwingUtilities.getWindowAncestor(contentPane);
        				win2.dispose();
                        break;
